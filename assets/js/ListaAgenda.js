...
function excluirEventoDaListEnviarLembrete(e) {
    ...
    var splitCookieEvento = docCookies.getItem('CookieListaAgendaEucatex').split(',');
    
    splitCookieEvento = $.grep(splitCookieEvento, function (value) {
        return value != excluirId;        
    });
    docCookies.setItem('CookieListaAgendaEucatex', splitCookieEvento);
        
    var splitCookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaEucatex").split(',');
    ...
    for (var i = 0; i < splitCookieLembrete.length; i++) {
        if (splitCookieLembrete[i].split('&D')[0] == excluirId) {
            ...
            docCookies.setItem('CookieEnviarLembreteAgendaEucatex', splitCookieLembrete);
        }
    }
...
function addEventoAListEnviarLembrete(e) {
    ...
    if (e.currentTarget.checked == true) {
        ...
        var cookieEvento = docCookies.getItem('CookieListaAgendaEucatex');
        ...
        if (typeof cookieEvento != 'undefined' && cookieEvento != "" && cookieEvento != null) { // Se cookie já tiver valor
            ...
            if ($.inArray(idEvento, splitCookieEvento) == -1) // Se idEvento NÃO estiver no cookie
            {
                ...
                var cookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaEucatex");  //  Adiciona o cookie 
                ...
                docCookies.setItem("CookieEnviarLembreteAgendaEucatex", splitCookieLembrete);  //  evento
            ...
            docCookies.setItem("CookieListaAgendaEucatex", idEvento);
            docCookies.setItem("CookieEnviarLembreteAgendaEucatex", idEvento + '&D' + diasParaLembrete);
...
function MarcarLembrete() {
    ...
    var getCookieLembrete = docCookies.getItem('CookieEnviarLembreteAgendaEucatex').split(',');
